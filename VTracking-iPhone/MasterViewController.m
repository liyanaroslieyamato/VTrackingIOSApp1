...
- (void)BarcodeScanViewController:(BarcodeScanViewController *)controller scanDone:(NSString *)scanResult
{
    ...
    if ([scanResult hasPrefix:@"YE"])
        filtered = [self.orders filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"(exchange_code == %@)", scanResult]];
    else
        filtered = [self.orders filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"(do_number == %@)", scanResult]];
    
    if ([filtered count] == 0) {
        filtered = [self.orders filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"do_number BEGINSWITH %@", scanResult]];
    }
    ...
    {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"There is some problem with this DO, please take a screenshot and contact admin"
                                                        message:scanResult
                                                       delegate:self
                                              cancelButtonTitle:@"OK"
                                              otherButtonTitles:nil];
...
- (void)download_orders {
    ...
    NSDictionary *queryParams = @{@"hpuser" : @"d"};
    
    [[RKObjectManager sharedManager] getObjectsAtPath:@"/hpapi/orders.json"
...
//    cell.textLabel.text = [[object valueForKey:@"timeStamp"] description];
    ...
    UILabel *do_number = (UILabel *)[cell viewWithTag:100];
    [do_number setText:order.do_number];
    
    UILabel *status = (UILabel *)[cell viewWithTag:200];
    [status setText:order.latest_status];

    if ([order.latest_status isEqualToString:@"Order Uploaded"]) {
        status.backgroundColor = [UIColor colorWithRed:0.83 green:0.26 blue:0.36 alpha:1];
    }
    else if ([order.latest_status isEqualToString:@"In Transit"]) {
        status.backgroundColor = [UIColor colorWithRed:0.27 green:0.53 blue:0.95 alpha:1];
    }
    else if ([order.latest_status isEqualToString:@"Return to Sender"]) {
        status.backgroundColor = [UIColor colorWithRed:0.48 green:0.46 blue:0.67 alpha:1];
    }
    else if ([order.latest_status isEqualToString:@"Tidiki Sorting Hub"]) {
        status.backgroundColor = [UIColor colorWithRed:0.95 green:0.51 blue:0.27 alpha:1];
    }
    else
    {
        status.backgroundColor = [UIColor colorWithRed:0.99 green:0.72 blue:0.21 alpha:1];
    }
    
    UILabel *address = (UILabel *)[cell viewWithTag:300];
    [address setText:order.r_address1];
